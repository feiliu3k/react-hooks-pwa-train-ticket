		...
		plugins: [
			...
			new ManifestPlugin({
				...
				generate: (seed, files, entrypoints) => {
					...
					// 处理build时 Cannot read property ‘filter’ of undefined报错
					const entryIndexpointFiles = entrypoints.index.filter(fileName => !fileName.endsWith('.map'))
					const entryQuerypointFiles = entrypoints.query.filter(fileName => !fileName.endsWith('.map'))
					const entryTicketpointFiles = entrypoints.ticket.filter(fileName => !fileName.endsWith('.map'))
					const entryOrderpointFiles = entrypoints.order.filter(fileName => !fileName.endsWith('.map'))

					return {
						files: manifestFiles,
						entrypoints: [...entryIndexpointFiles, ...entryQuerypointFiles, ...entryTicketpointFiles, ...entryOrderpointFiles],
					}
				},
			}),
